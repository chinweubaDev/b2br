    ...
    <div x-data="{ open: false }" class="relative">
        <button @click="open = !open" class="flex items-center gap-2 hover:opacity-80 transition">
            ...
            <div class="hidden md:block text-left">
                <div class="text-sm font-medium text-gray-200">{{ auth()->user()->name ?? 'Admin' }}</div>
        ...
        <div x-show="open" 
            ...
            <div class="p-4 border-b border-gray-700">
                <div class="font-medium text-gray-200">{{ auth()->user()->name ?? 'Admin' }}</div>
                <div class="text-sm text-gray-400 mt-1">{{ auth()->user()->email ?? 'admin@example.com' }}</div>
            </div>
            <div class="p-2">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white hover:bg-opacity-5 transition text-gray-300 hover:text-white">
                    <i class="fas fa-home w-4"></i>
                    <span class="text-sm">Main Site</span>
                </a>
                <a href="{{ route('admin.settings') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white hover:bg-opacity-5 transition text-gray-300 hover:text-white">

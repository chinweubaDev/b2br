...
class TourController extends Controller
{
    ...
    $request->file('image')->move(public_path('storage/tours'), $filename);
    $validated['image'] = '/tours/' . $filename; // save clean path
}

if ($request->hasFile('featured_image')) {
    $filename = uniqid() . '.' . $request->file('featured_image')->extension();
    $request->file('featured_image')->move(public_path('storage/tours'), $filename);
    $validated['featured_image'] = '/tours/' . $filename; // save clean path
}
...
if ($request->hasFile('extra_images')) {
    foreach ($request->file('extra_images') as $index => $image) {
        if ($image && $image->isValid()) {
            ...
            $image->move(public_path('storage/tours/extra'), $filename);
            $imagePath = '/tours/extra/' . $filename; // save clean path
